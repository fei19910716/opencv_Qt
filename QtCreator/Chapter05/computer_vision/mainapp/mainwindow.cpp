...
#define PLUGINS_SUBFOLDER                   "/resources/cvplugins/"
#define LANGUAGES_SUBFOLDER                 "/resources/languages/"
#define THEMES_SUBFOLDER                    "/resources/themes/"
...
void MainWindow::populatePluginsMenu()
{
    ...
    foreach(QFileInfo pluginFile, pluginFiles)
    {
        if(QLibrary::isLibrary(pluginFile.absoluteFilePath()))
        {
            ...
            {
                QAction *pluginAction = ui->menu_Plugins->addAction(plugin->title()); // 添加plugin菜单
